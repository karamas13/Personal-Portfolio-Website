...
const ProjectDetail = () => {
  ...
  return (
    <div className="min-h-screen scrollbar-thin scrollbar-thumb-purple-900 scrollbar-track-stone-800 h-32 overflow-y-scroll overflow-x-hidden">
      <Helmet>
        <title>{project.title} - Nick Karamaroudis | Full Stack Developer</title>
        <meta name="description" content={`Learn about the ${project.title} project by Nick Karamaroudis. This project involves ${project.technologies.join(", ")} and showcases my skills as a Full Stack Developer.`} />
        <meta name="keywords" content={`Project, ${project.title}, Full Stack Developer, Nick Karamaroudis, ${project.technologies.join(", ")}`} />
      </Helmet>
      
      <div className="top-0 left-0 text-purple-600 text-5xl m-2 cursor-pointer hover:text-purple-900 ease-in-out duration-[0.5s]" onClick={() => { nav(-1); }}>
        <AiOutlineArrowLeft />
      </div>
        ...
        <div className="lg:p-4 mt-10">
          ...
          <div className='flex items-center lg:flex-col flex-col'>
            <motion.img 
              ...
              src={project.image} 
              alt={project.title} 
              className="my-10 lg:h-[25rem] h-[15rem] w-auto object-cover rounded-xl mr-5" 
            />
            <motion.div 
              animate={{opacity: 1, y: 0}}
              initial={{opacity: 0, y: 100}}
              ...
              <a href={project.link} target="_blank" rel="noopener noreferrer"> 
                <motion.button 
                  ...
                  className='text-2xl mt-20 border lg:w-[100%] lg:p-1 lg:px-5 p-2 rounded-2xl border-purple-600 text-purple-600 font-bold'
                >
                  Visit
                </motion.button>
              </a>
